...
class UserApiControllerTest extends Specification {
    @Autowired
    MockMvc mvc
    @Autowired
    ...
    def "findAll"() {
        ...
        when(userService.findAll()).thenReturn(List.of(user(1, createdAt, updatedAt),
                user(2, createdAt, updatedAt)))
    ...
    def "create"() {
        ...
        def createdAt = LocalDateTime.now()
        def updatedAt = createdAt.plusHours(1)
        def request = new UserApiController.UserRequest()
        request.name = name
